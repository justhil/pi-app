...
Abstracts differences between Claude Code, OpenCode, Cursor, iFlow, Agent 桌面, Kilo, Kiro Code, Gemini CLI, Antigravity, Windsurf, Qoder, CodeBuddy, GitHub Copilot, Factory Droid, and Pi Agent interfaces.
...
- cursor: Cursor IDE
- iflow: iFlow CLI
- codex: Agent 桌面 CLI (skills-based)
- kilo: Kilo CLI
...
class CLIAdapter:
    ...
    def get_agent_path(self, agent: str, project_root: Path) -> Path:
        ...
        Returns:
            Path to agent definition file (.md for most platforms, .toml for Agent 桌面)
        """
    ...
    def get_trellis_command_path(self, name: str) -> str:
        ...
        Note:
            ...
            Agent 桌面: .agents/skills/trellis-<name>/SKILL.md
    ...
    def supports_cli_agents(self) -> bool:
        ...
        Claude Code, OpenCode, iFlow, and Agent 桌面 support CLI agent execution.
    ...
    def requires_agent_definition_file(self) -> bool:
        ...
        Agent 桌面: auto-discovers agents from .codex/agents/*.toml, no --agent flag.
        """
...
layer (written by Agent 桌面, also consumed by Amp/Cline/Warp/etc. via the
...
def detect_platform(project_root: Path) -> Platform:
    ...
    # Check for .codex directory (Agent 桌面-specific)
    ...
    # Fallback: checkout only has the Agent 桌面 shared-skills layer
