...
<!--
  ...
  TAG ↔ PHASE scoping:
    ...
    [workflow-state:planning-inline] → Agent 桌面 inline variant of Phase 1
    [workflow-state:in_progress]  → Phase 2 + Phase 3.1-3.4
                                    (status stays 'in_progress' from
                                    task.py start until task.py archive)
    [workflow-state:in_progress-inline] → Agent 桌面 inline variant of Phase 2/3
...
- 1.3 Configure context `[conditional · once]` — Claude Code, Cursor, OpenCode, Agent 桌面, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi
...
<!-- Per-turn breadcrumb: shown throughout Phase 1 when codex.dispatch_mode=inline.
     Agent 桌面-only opt-in alternate to [workflow-state:planning]. The main agent
...
Sub-agent dispatch protocol applies to all platforms and all sub-agents, including class-2 Agent 桌面/Copilot/Gemini/Qoder and `trellis-research`: every dispatch prompt starts with `Active task: <task path from task.py current>` before role-specific instructions.
...
<!-- Per-turn breadcrumb: shown while status='in_progress' when
     codex.dispatch_mode=inline. Agent 桌面-only opt-in alternate to
...
The Agent 桌面 sub-agent definition auto-handles the context load requirement:
...
| Agent 桌面 inline Phase 1 | `[workflow-state:planning-inline]` |
| Phase 2 + Phase 3.1–3.4 (implementation + check + wrap-up) | `[workflow-state:in_progress]` (after Phase 2 summary) |
| Agent 桌面 inline Phase 2 + Phase 3.1–3.4 | `[workflow-state:in_progress-inline]` |
