...
### 决策记录：外部同步用三态指示器，移除发送前确认弹窗（2026）

外部更新后不再弹「重载确认」弹窗、发送前也不再拦截确认——实时同步已把磁盘尾部并入视图，弹窗冗余且打断输入。改为 composer 上方**三态指示器**（`externalSyncPhase` 驱动，store 不再存 `externalUpdateFor`）：外部写入合并成功 → **绿色脉冲「正在同步外部对话」**，约 5s 无外部写入自动隐藏（本轮结束）；IPC 同步异常 → **橙/红「外部同步异常」**，点击触发完整重载；空闲不渲染。**实现注意**：zustand `setState` 返回 `undefined`，判断“是否有新增”必须在 updater 内用局部变量捕获，不能靠返回值（曾导致绿灯从不亮）。

### 决策记录：活动窗口在 tool-group 粒度生效（2026）

工具行合并成 tool-group 折叠块后，组展开原本只受点击控制、窗口预算只传给组内子行（组不展开则子行永远不可见，表现为“窗口无效”）。修复：组 open = 手动状态（三态，null=未手动）?? 组内任一工具行命中窗口预算；组展开后子行（含组内 thinking）跟随展开。窗口在组粒度生效：命中预算的组整组可见。

### TODO：实时思考过程同步（等待 pi 支持流式落盘）

pi CLI 的会话 JSONL 是**消息级落盘**（`agent-session.js` “saves messages on message_end”；磁盘无流式中间条目，thinking+text+toolCall 同一条消息一次性 append）。因此 app 同步 CLI 会话的粒度上限 = 每条消息完成，CLI 思考过程中 app 看不到中间状态。**等 pi 支持思考块逐块落盘后**再回来做实时思考同步；当前替代方案：在 app 内发消息（worker 流式实时渲染）。勿再优化 watcher debounce/轮询间隔（那是消息级延迟，不是思考级）。
