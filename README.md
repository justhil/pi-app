...
- **做**：单活动工作目录、会话列表、流式对话、工具时间线、改动审查、运行/上下文侧栏、Trellis 只读、会话树跳转、沙箱临时对话、扩展与适配器配置。
- **不做**：云端委派、多项目并行工作台、团队协作、完整 IDE、可交互终端仿真、自建一套权限审批替代 pi、在应用里改 Trellis 任务状态。

### 5. 体验取向

- **工具型 Agent 工作台**：对话和执行过程可见，密度适中，偏开发者桌面客户端，而不是营销型聊天页。
- 侧栏可收拢、对话区随窗口伸缩、工具结果默认折叠、流式输出用 Markdown 与可折叠思考链呈现。
