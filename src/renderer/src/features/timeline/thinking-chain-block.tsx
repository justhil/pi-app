...
export function ThinkingChainBlock({
  ...
  return (
    <div className="mb-1.5">
      ...
      >
        <ChevronRight
          className={cn('chevron-expand h-3 w-3 text-foreground-secondary/45', open && 'rotate-90')}
        />
        <span className="text-[11px] text-foreground-secondary/50">
          {streaming ? t('timeline:thinkingActive') : t('timeline:thoughtDone')}
        </span>
      </button>
      <CollapsiblePanel open={open} className="mt-0.5">
        <div className="ml-4 max-h-48 overflow-auto border-l border-border/25 pl-2 text-[12px] leading-[1.55] text-foreground-secondary/45 whitespace-pre-wrap break-words font-mono">
          {open ? text : preview}
        </div>
      </CollapsiblePanel>
    </div>
