...
function handleMessage(event: Extract<AppEvent, { type: 'message' }>, api: StoreApi): void {
  const state = api.get()
...
function handleTool(event: Extract<AppEvent, { type: 'tool' }>, api: StoreApi): void {
  const state = api.get()
  if (event.phase === 'start') {
...
function handleRun(event: Extract<AppEvent, { type: 'run' }>, api: StoreApi): void {
  const state = api.get()
...
function handleCompaction(event: Extract<AppEvent, { type: 'compaction' }>, api: StoreApi): void {
  const state = api.get()
  if (event.phase === 'start') {
    void Promise.all([
...
function handleSlash(event: Extract<AppEvent, { type: 'slash' }>, api: StoreApi): void {
  const state = api.get()
...
function handleAgentError(event: Extract<AppEvent, { type: 'agent_error' }>, api: StoreApi): void {
  const state = api.get()
